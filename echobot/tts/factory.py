...
def build_default_gpt_sovits_tts_provider() -> GPTSoVITSTTSProvider:
    return GPTSoVITSTTSProvider(
        api_url=_env_text(
            "ECHOBOT_TTS_GPT_SOVITS_API_URL",
            "http://127.0.0.1:9880/tts",
        ),
        ref_audio_path=_env_text(
            "ECHOBOT_TTS_GPT_SOVITS_REF_AUDIO",
            "voices/CRS_JP.wav",
        ),
        prompt_text=_env_text(
            "ECHOBOT_TTS_GPT_SOVITS_PROMPT_TEXT",
            "極端な管理社会全体主義まゆりがバナナを食べたいと思っても、今日がバナナを食べていい日でなければ食べることは許さ。",
        ),
        prompt_lang=_env_text("ECHOBOT_TTS_GPT_SOVITS_PROMPT_LANG", "ja"),
        text_lang=_env_text("ECHOBOT_TTS_GPT_SOVITS_TEXT_LANG", "ja"),
        timeout=max(1.0, _env_float("ECHOBOT_TTS_GPT_SOVITS_TIMEOUT", 60.0)),
        speed_factor=max(0.1, _env_float("ECHOBOT_TTS_GPT_SOVITS_SPEED", 0.9)),
    )
