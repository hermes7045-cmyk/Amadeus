---
name: amadeus-skill-authoring
description: Use when creating, revising, or validating a skill for this Amadeus repository, including SKILL.md frontmatter, trigger descriptions, project skill folders under skills/, bundled references or scripts or assets or agents, or Amadeus skill discovery, explicit activation, and lazy resource loading. Also use for requests like “更新 skill”, “写 SKILL.md”, “这个 skill 为什么没触发”, or “给 Amadeus 加一个项目内技能”.
---

# Amadeus Skill Authoring

Create or revise repository-local skills that Amadeus can discover and activate.
...
- Keep frontmatter minimal. Amadeus routing reads `name` and `description` from `SKILL.md`.
...
- Design for lazy loading. After activation, Amadeus exposes the body plus a resource summary. It does not auto-load bundled files.
...
- Do not treat `agents/openai.yaml` as required. Current Amadeus discovery ignores it.
...
- Amadeus currently ignores extra frontmatter keys. The validator is more permissive for compatibility, but project-local skills should usually keep only `name` and `description`.
...
5. Validate the skill with `python -X utf8 amadeus/skills/skill-creator/scripts/quick_validate.py skills/<skill-name>`.
6. If the skill changes runtime expectations, update tests.

## Amadeus-specific notes
...
- If you change discovery, parsing, or activation behavior in `amadeus/skill_support/`, run `python -m unittest tests.test_skill_support tests.test_chat_agent tests.test_agent -v`.
