...
Read this before changing `amadeus/skill_support/` or designing a skill that relies on resource files.
...
- Amadeus currently routes only on top-level `name` and `description`.
...
- The external quick validator accepts some extra frontmatter keys for compatibility, but Amadeus runtime behavior still only depends on `name`, `description`, and the body text.
...
- If the user explicitly writes `/skill-name` or `$skill-name`, Amadeus injects that skill immediately unless it is already active.
...
- `agents/openai.yaml` is not required for Amadeus skill discovery. Only add agent files when some explicit workflow will read them.
