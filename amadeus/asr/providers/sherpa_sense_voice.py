...
class SenseVoiceModelManager:
    def __init__(
        ...
        *,
        ...
        root_dir = model_root_dir or (
            workspace / ".amadeus" / "models" / "asr" / "sherpa-sense-voice"
        )
    ...
    def _install_model(self) -> None:
        with tempfile.TemporaryDirectory(prefix="amadeus_sense_voice_") as temp_dir:
    ...
    async def _maybe_start_prepare(self) -> None:
        async with self._status_lock:
            ...
            self._prepare_task = asyncio.create_task(
                self._prepare_model(),
                name="amadeus_sense_voice_prepare",
            )
