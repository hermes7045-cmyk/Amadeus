...
class OpenAITranscriptionsASRProvider(ASRProvider):
    ...
    def _status_state(self) -> tuple[str, str]:
        ...
        if not self._base_url:
            return "missing", "OpenAI transcriptions provider is missing AMADEUS_ASR_OPENAI_BASE_URL."
        if not self._model:
            return "missing", "OpenAI transcriptions provider is missing AMADEUS_ASR_OPENAI_MODEL."
        if self._uses_official_openai_endpoint() and self._api_key.upper() in {"", "EMPTY"}:
            return "missing", "OpenAI official transcription endpoint requires AMADEUS_ASR_OPENAI_API_KEY."
        return (
            "ready",
