"""Gateway セッションサービス — コアセッションとルートセッション・配信先を統合。"""
...
class GatewaySessionService:
    """SessionLifecycleService と RouteSessionStore / DeliveryStore を束ねる統合サービス。"""
    def __init__(
        self,
    ...
    async def rename_session(self, old_name: str, new_name: str):
        """セッション名を変更し、RouteSessionStore と DeliveryStore も追従させる。"""
    ...
    async def delete_session(self, name: str) -> bool:
        """セッションを削除し、関連するルートセッション・配信情報も除去する。"""
    ...
    async def delete_current_route_session(
        ...
        """指定ルートの現在のルートセッションを削除し、コアセッションもパージする。"""
