...
DEFAULT_DELIVERY_STORE_PATH = Path(".amadeus/delivery.json")


@dataclass(slots=True)
class DeliveryState:
    """配信先状態 — セッション名→配送先のマッピングと最新セッション名。"""
...
class DeliveryStore:
    ...
    def replace_session_name(self, old_name: str, new_name: str) -> bool:
        """配信先のセッション名を old_name から new_name に置き換える。"""
        with self._lock:
            self._ensure_loaded()
