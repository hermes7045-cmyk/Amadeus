"""ルート単位のセッション管理 — セッションの作成・切替・保存・復元。"""
...
DEFAULT_ROUTE_SESSION_STORE_PATH = Path(".amadeus/route_sessions.json")
...
class RouteSessionSummary:
    """ルートセッションのサマリー情報（名前・タイトル・作成/更新日時）。"""
    session_name: str
    title: str
    created_at: str
    updated_at: str
...
class RouteSessionState:
    """単一ルートのセッション状態（現在のセッション名＋セッション一覧）。"""
...
class DeleteRouteSessionResult:
    """セッション削除の結果（削除されたセッション＋新しい現在セッション）。"""
...
class RouteSessionStore:
    """ルートセッションの永続ストア — JSON ファイルによる保存・復元。"""
    def __init__(
        self,
    ...
    def get_current_session(self, route_key: str) -> RouteSessionSummary:
        """指定ルートの現在のセッションを取得する。存在しなければ新規作成する。"""
        with self._lock:
            self._ensure_loaded()
    ...
    def remove_session(self, session_name: str) -> bool:
        """全ルートから指定セッション名を削除する。変更があれば True を返す。"""
        with self._lock:
            self._ensure_loaded()
            changed = False
    ...
    def replace_session_name(self, old_name: str, new_name: str) -> bool:
        """全ルートのセッション名を old_name から new_name に置き換える。"""
        with self._lock:
            self._ensure_loaded()
            changed = False
