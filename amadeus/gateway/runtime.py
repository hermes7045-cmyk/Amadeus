"""Gateway ランタイム — メッセージ受信・セッション管理・応答配信の制御。"""
...
class GatewayRuntime:
    """Gateway のメインループ — インバウンドメッセージを処理しアウトバウンド応答を配信する。"""
    def __init__(
        self,
        ...
        *,
        ...
        if session_service is None:
            delivery_store = delivery_store or DeliveryStore(
                context.workspace / ".amadeus" / "delivery.json",
            )
            route_session_store = route_session_store or RouteSessionStore(
                context.workspace / ".amadeus" / "route_sessions.json",
            )
    ...
    async def run(self) -> None:
        """メッセージバスからインバウンドメッセージを消費し続けるメインループ。"""
    ...
    async def handle_inbound_message(self, message: InboundMessage) -> None:
        """1件のインバウンドメッセージをルート単位のロック下で処理する。"""
