...
export function createAsrModule(deps) {
    ...
    async function handleUnexpectedSocketClose() {
        if (!asrState.alwaysListenEnabled) {
            return;
        }
        ...
        while (retryCount < MAX_RETRIES) {
            retryCount += 1;
            const delay = Math.min(1000 * (1 << (retryCount - 1)), 30000);
            setConnectionState("reconnecting", "实时语音重新连接中...");
            addSystemMessage("实时语音重新连接中...");
            ...
            try {
                ...
                addSystemMessage("实时语音已重新连接");

                asrState.alwaysListenEnabled = true;
                asrState.microphoneCaptureMode = "always";
                if (DOM.alwaysListenCheckbox) {
                    DOM.alwaysListenCheckbox.checked = true;
                }
                updateVoiceInputControls();
                return;
            } catch (error) {
                console.warn("ASR reconnection attempt %d failed", retryCount, error);
            }
        }
