...
async def list_roles(runtime=Depends(get_app_runtime)) -> list[RoleSummaryModel]:
    all_cards = await runtime.role_service.list_roles()
    non_default_cards = [card for card in all_cards if card.name != DEFAULT_ROLE_NAME]
    # Only exclude default when other roles exist (keep fallback)
    if non_default_cards:
        return [
            _summary_model_from_role_card(card)
            for card in non_default_cards
        ]
    return [
        _summary_model_from_role_card(card)
