...
def build_runtime_context(
    ...
    *,
    ...
    lightweight_max_tokens = _env_int("AMADEUS_LIGHTWEIGHT_MAX_TOKENS", 4096)
    agent_max_steps = _env_int("AMADEUS_AGENT_MAX_STEPS", 50)
    settings = OpenAICompatibleSettings.from_env()
    supports_image_input = _env_bool("AMADEUS_LLM_SUPPORTS_IMAGE_INPUT", True)
    attachment_store = AttachmentStore(
        workspace / ".amadeus" / "attachments",
    ...
    agent_trace_store = AgentTraceStore(workspace / ".amadeus" / "agent_traces")
    ...
    job_store = ConversationJobStore(workspace / ".amadeus" / "jobs" / "jobs.json")
...
def _heartbeat_file_path(workspace: Path) -> Path:
    file_name = os.environ.get(
        "AMADEUS_HEARTBEAT_FILE",
        ".amadeus/HEARTBEAT.md",
    )
...
def _heartbeat_interval_seconds(options: RuntimeOptions) -> int:
    ...
    raw_value = os.environ.get("AMADEUS_HEARTBEAT_INTERVAL_SECONDS", "1800")
    try:
        value = int(raw_value)
    except ValueError:
        value = 1800
...
def _heartbeat_enabled() -> bool:
    raw_value = os.environ.get("AMADEUS_HEARTBEAT_ENABLED", "true").strip().lower()
...
def _delegated_ack_enabled(
    ...
    *,
    ...
    return _env_bool("AMADEUS_DELEGATED_ACK_ENABLED", True)
...
def _shell_safety_mode(runtime_settings) -> str:
    ...
    raw_value = os.environ.get("AMADEUS_SHELL_SAFETY_MODE", "").strip().lower()
    if raw_value:
        return raw_value
...
def _image_budget_from_env() -> ImageBudget:
    ...
    return ImageBudget(
        max_input_bytes=_env_int(
            "AMADEUS_IMAGE_MAX_INPUT_BYTES",
            defaults.max_input_bytes,
        ),
        max_output_bytes=_env_int(
            "AMADEUS_IMAGE_MAX_OUTPUT_BYTES",
            defaults.max_output_bytes,
        ),
        max_side=_env_int(
            "AMADEUS_IMAGE_MAX_SIDE",
            defaults.max_side,
        ),
        max_pixels=_env_int(
            "AMADEUS_IMAGE_MAX_PIXELS",
            defaults.max_pixels,
        ),
...
def _runtime_settings_path(workspace: Path) -> Path:
    return workspace / ".amadeus" / "runtime_settings.json"
