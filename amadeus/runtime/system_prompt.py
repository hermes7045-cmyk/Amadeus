...
def _build_identity_section(workspace_path: Path) -> str:
    ...
    lines = [
        "# Amadeus",
        "",
        "You are Amadeus, the full tool-using agent operating inside the user's project workspace.",
        "",
        "## Environment",
        f"- OS: {system_name} {release_name}",
        f"- Python: {python_version}",
        f"- Workspace: {workspace_path}",
        f"- Session store: {workspace_path / '.amadeus' / 'sessions'}",
        "",
    ]
...
def _build_scheduling_section(
    ...
    *,
    ...
    cron_path = (
        ...
        else workspace_path / ".amadeus" / "cron" / "jobs.json"
    )
    heartbeat_path = (
        Path(heartbeat_file_path).resolve()
        if heartbeat_file_path is not None
        else workspace_path / ".amadeus" / "HEARTBEAT.md"
    )
    interval_text = (
