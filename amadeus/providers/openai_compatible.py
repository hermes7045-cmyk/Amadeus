...
class OpenAICompatibleSettings:
    ...
    def from_env(
        ...
        *,
        ...
        verify_ssl_text = _get_optional_env(
            source,
            f"{prefix}VERIFY_SSL",
            default="true",
        )
        verify_ssl = verify_ssl_text.strip().lower() not in {"0", "false", "no", "off"}
...
class OpenAICompatibleProvider(LLMProvider):
    def __init__(
        ...
        *,
        ...
        # --- prompt caching (system prompt repeat detection) ---
        self._last_system_hash: str = ""
        self._system_repeat_count: int = 0
        # --- response cache ---
    ...
    async def generate(
        ...
        *,
        ...
        # --- response cache check (short identical requests) ---
        cache_key = ResponseCache.make_key(payload)
        cached_raw = self._response_cache.get(cache_key)
        if cached_raw is not None:
            logger.debug("Response cache hit for payload %s", cache_key[:12])
            return self._parse_response(cached_raw)

        response_data = await asyncio.to_thread(self._post_json, payload)
        self._response_cache.set(cache_key, response_data)
    ...
    def _build_payload(
        ...
        *,
        ...
        # --- track system prompt hash for prompt caching ---
        self._track_system_prompt(payload)

        return payload

    def _track_system_prompt(self, payload: dict[str, Any]) -> None:
        """Detect repeated system prompts via SHA256 hash.

        Updates ``_system_repeat_count`` and ``_last_system_hash``
        so that ``_post_json`` / ``_stream_text_chunks`` can add
        caching hints on the 2nd+ consecutive request.
        """
        ...
        current_hash = hashlib.sha256(system_text.encode("utf-8")).hexdigest()
        if current_hash and current_hash == self._last_system_hash:
    ...
    def _post_json(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        # On 2nd+ consecutive request with the same system prompt,
        # signal the provider that this prefix is cacheable.
        if self._system_repeat_count >= 2:
            headers["X-Prompt-Cache"] = "HIT"
            # Some OpenAI-compatible providers accept an extra_body
            # hint for prompt caching.
            if "prompt_caching" not in payload:
                payload["prompt_caching"] = {"enabled": True}
            logger.debug(
                "Prompt cache active for %s (repeat #%d)",
                self.settings.model,
                self._system_repeat_count,
            )
        ...
        try:
            with request.urlopen(http_request, timeout=self.settings.timeout, context=self._ssl_context) as response:
                return json.loads(response.read(10 * 1024 * 1024).decode("utf-8"))
